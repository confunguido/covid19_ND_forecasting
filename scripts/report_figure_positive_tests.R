...
download.file('https://covidtracking.com/api/v1/states/daily.csv',
              '../../data/US_states_testing_data.csv')

us_testing = read_csv('../../data/US_states_testing_data.csv') %>%
    dplyr::select(date, state, positiveIncrease, negativeIncrease) %>%
    rename(positive=positiveIncrease,negative=negativeIncrease,State=state) %>%
...
us_states = us_states_temp %>%
    ...
    left_join(us_testing,by = c("date", "State")) %>%
    group_by(State) %>%
...
jpeg('../figures/report_figure_positive_tests.jpeg', width=7,height=5, units="in", res = 300)
...
for(nn in 1:nrow(interventions_df)){
    ...
    state_fred$prevalence = state_fred$Is_mean / state_fred$N_mean
    state_fred$infected_prev = (state_fred$I_mean - state_fred$Is_mean) / state_fred$N_mean
    
    state_fred$probC = state_fred$prevalence / (state_fred$prevalence + state_fred$tests_ratio * (1 - state_fred$prevalence))
    
    state_fred$probI = state_fred$infected_prev*state_fred$tests_ratio / (state_fred$prevalence + state_fred$tests_ratio * (1 - state_fred$prevalence))
    state_fred$probU = (1 - state_fred$infected_prev - state_fred$prevalence)*state_fred$tests_ratio / (state_fred$prevalence + state_fred$tests_ratio * (1 - state_fred$prevalence))    
    
    state_fred$pos_test = (state_fred$probC + state_fred$probI) * sens + state_fred$probU*(1 - spec)
    ...
    tmp_fred = intervention_fred %>%
        ...
        summarize(pos_test_median = quantile(pos_test, probs = c(0.5), na.rm = T),
                  pos_test_low = quantile(pos_test, probs = c(0.025), na.rm = T),
                  pos_test_high = quantile(pos_test, probs = c(0.975), na.rm = T)) %>%
        ungroup()

    plot(tmp_fred$Date, tmp_fred$pos_test_median, xaxs = "i", yaxs = "i", type = "l", lwd = 2, col = col_palette[nn],
         xlab = "", ylab = "", xlim = c(times_to_plot[1],times_to_plot[length(times_to_plot)]),
         ylim = c(0,1.0), xaxt = 'n', yaxt = 'n')
    mtext(side = 3, text = ss)
    axis(side = 2, las = 2, at = seq(0, 1.0, length.out = 6),
         labels = signif(seq(0, 1.0, length.out = 6), 2))
    ...
    polygon(x = c(tmp_fred$Date, rev(tmp_fred$Date)),
            y = c(tmp_fred$pos_test_high,
                    rev(tmp_fred$pos_test_low)),
            border = adjustcolor(col_palette[nn], alpha.f = 0.7),
            col = adjustcolor(col_palette[nn], alpha.f = 0.2))    
    lines(tmp_fred$Date, tmp_fred$pos_test_median, lwd = 1.5, 
          col = adjustcolor(col_palette[nn], alpha.f = 0.2)) 
    ...
    points(tmp_data_fit$date, tmp_data_fit$positive / (tmp_data_fit$positive + tmp_data_fit$negative), col = "navy", lwd = 0.3,pch = 18)
}

mtext("Proportion of positive tests", side = 2, line = 2, outer = T)
